...
impl AccountAccountStore {
    pub fn parse_data(
        ...
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
...
impl EthereumTransactionIndex {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<(H256, (U256, u32))> {
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
            (kv_pair.0.to_vec(), kv_pair.1.to_vec())
        ...
        Ok((
            ...
            serde_json::from_slice::<(U256, u32)>(&value).c(d!())?,
        ))
    }
}

impl EthereumCurrentBlockNumber {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<U256> {
        let val = if self.decode_kv {
...
impl EthereumBlockHash {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<(U256, H256)> {
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
            (kv_pair.0.to_vec(), kv_pair.1.to_vec())
...
impl EthereumCurrentBlock {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<(H256, Block)> {
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
            (kv_pair.0.to_vec(), kv_pair.1.to_vec())
...
impl EthereumCurrentReceipts {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<(H256, Vec<Receipt>)> {
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
            (kv_pair.0.to_vec(), kv_pair.1.to_vec())
...
impl EVMAccountCodes {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<(H160, Vec<u8>)> {
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
            (kv_pair.0.to_vec(), kv_pair.1.to_vec())
...
impl EVMAccountStorages {
    pub fn parse_data(&self, kv_pair: &(Box<[u8]>, Box<[u8]>)) -> Result<((H160, H256), H256)> {
        let (key, value) = if self.decode_kv {
            decode_kv(&kv_pair)
        } else {
            (kv_pair.0.to_vec(), kv_pair.1.to_vec())
        ...
        let key1 = *key_list.get(1).c(d!())?;
        let key2 = *key_list.get(2).c(d!())?;
        Ok((
